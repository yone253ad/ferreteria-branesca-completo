...
// 2. Importaciones de Iconos (Agregado 'Briefcase' para Clientes)
import { 
  LayoutDashboard, Package, ShoppingCart, Users, FileText, 
  Menu, Bell, ChevronDown, LogOut, Landmark, Scissors, Warehouse, Briefcase
} from 'lucide-react';
...
function AdminLayout() {
  ...
  const renderMenuContent = () => (
    <>
        ...
        {/* CLIENTES (NUEVO LINK) */}
        {['ADMIN', 'GERENTE', 'VENDEDOR'].includes(rol) && (
          <Link to="/clientes" className={`compact-menu-item ${isActive('/clientes')}`} onClick={() => setShowMobileMenu(false)}>
            <div className="menu-icon"><Briefcase size={20} /></div>
            <span className="menu-label">Cartera Clientes</span>
          </Link>
        )}
        ...
        {rol === 'ADMIN' && (
          <Link to="/usuarios" className={`compact-menu-item ${isActive('/usuarios')}`} onClick={() => setShowMobileMenu(false)}>
            ...
            <span className="menu-label">Personal</span>
          </Link>
        )}
    ...
    <div className="admin-container">
                ...
                <Dropdown align="end">
                  ...
                  <Dropdown.Menu className="shadow border-0 p-0" style={{ width: '300px', maxHeight: '400px', overflowY: 'auto' }}>
                    <div className="p-3 border-bottom d-flex justify-content-between align-items-center">
                        ...
                        {notificationsList.length > 0 && (
                            <small className="text-primary" style={{cursor:'pointer'}} onClick={clearNotifications}>Borrar todo</small>
                        )}
                    </div>
                    {notificationsList.length === 0 ? (
                        <div className="p-4 text-center text-muted"><small>No hay notificaciones nuevas</small></div>
                    ) : (
                        notificationsList.map((notif) => (
                            <Dropdown.Item key={notif.id} as={Link} to="/pedidos" className="border-bottom py-2">
                                <div className="d-flex align-items-start">
                                    <div className="bg-light p-2 rounded-circle me-2 text-success"><ShoppingCart size={16} /></div>
                                    <div>
                                        <strong className="d-block text-dark" style={{fontSize: '0.9rem'}}>Nuevo Pedido #{notif.orderId}</strong>
      ...
      <ToastContainer position="top-end" className="p-3" style={{ zIndex: 9999, position: 'fixed', top: '60px' }}>
        <Toast onClose={() => setShowToast(false)} show={showToast} delay={8000} autohide bg="success">
          <Toast.Header closeButton><strong className="me-auto text-success">🔔 Nuevo Pedido</strong></Toast.Header>
          <Toast.Body className="text-white">
            Pedido <strong>#{notificationData.id}</strong> recibido.<br/>
            <Button size="sm" variant="light" as={Link} to="/pedidos" className="mt-2" onClick={() => setShowToast(false)}>Ver Pedidos</Button>
          </Toast.Body>
        </Toast>
      </ToastContainer>
    </div>
